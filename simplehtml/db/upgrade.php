 ...
 * @param object $block
 */
function xmldb_block_simplehtml_upgrade($oldversion, $block) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2018100500) {
        ...
        // Define table block_prba to be created.
        $table = new xmldb_table('block_prba');

        // Adding fields to table block_prba.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);

        // Adding keys to table block_prba.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);

        // Conditionally launch create table for block_prba.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Simplehtml savepoint reached.
        upgrade_block_savepoint(true, 2018100500, 'simplehtml');
